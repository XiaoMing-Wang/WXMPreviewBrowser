...
@interface WXMPrePhotoImageView ()
@property (nonatomic, strong) WXMPrePhotoGIFImage *animatedImage;
...
@implementation WXMPrePhotoImageView
const NSTimeInterval kMaxTimeSteppre = 1;
...
- (CADisplayLink *)displayLink {
    if (!self.gifAnimating) return nil;
    if (self.superview) {
...
- (void)setImage:(UIImage *)image {
    ...
    if ([image isKindOfClass:[WXMPrePhotoGIFImage class]] && image.images) {
        if ([image.images[0] isKindOfClass:UIImage.class]) [super setImage:image.images[0]];
        else [super setImage:nil];
        self.currentFrame = nil;
        self.animatedImage = (WXMPrePhotoGIFImage *) image;
...
- (void)changeKeyframe:(CADisplayLink *)displayLink {
    if (self.currentFrameIndex >= [self.animatedImage.images count]) return;
    self.accumulator += fmin(displayLink.duration, kMaxTimeSteppre);
